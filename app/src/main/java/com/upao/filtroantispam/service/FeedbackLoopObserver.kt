...
object FeedbackLoopObserver {
    ...
    private fun procesarUltimaLlamada(context: Context) {
        ...
        try {
            // "LIMIT 1" en el sortOrder funcionaba en versiones viejas de Android,
            // pero el proveedor de CallLog en versiones más nuevas lo rechaza
            // (IllegalArgumentException: Invalid token LIMIT) y esto se dispara
            // en CADA llamada entrante/saliente: sin este fix, procesarUltimaLlamada
            // crasheaba la app cada vez que colgabas. cursor.moveToFirst() ya
            // basta para quedarnos solo con la primera fila.
            context.contentResolver.query(
                ...
                FiltroDatabase.obtenerInstancia(context).reporteLocalDao().incrementarNormalizado(numero)
                Log.d(TAG, "Llamada corta de número desconocido ($numero, ${duracion}s): reforzando patrón de spam")
            }
        } catch (e: Exception) {
            // No solo SecurityException: el cursor del CallLog (columnas, tipos)
            // puede variar entre fabricantes/Android versions, y esto corre en
            // el hilo principal disparado por el ciclo de vida de la llamada.
            // Una excepción sin capturar aquí tumba el proceso entero justo
            // cuando el usuario acaba de colgar.
            Log.w(TAG, "Error procesando el feedback loop, se ignora esta llamada", e)
        }
    }
}
